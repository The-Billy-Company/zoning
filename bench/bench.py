...
def main() -> int:
    ...
    ap.add_argument(
        "--reps", type=int, default=3, help="timed runs per tool, best wins"
    )
    ap.add_argument(
        "--no-uv", action="store_true", help="only race rivals already on PATH"
    )
    ap.add_argument(
        "--json", action="store_true", help="one JSON record instead of tables"
    )
    args = ap.parse_args()
...
def correctness(
    tools: dict[str, rivals.Launcher],
...
def speed(
    tools: dict[str, rivals.Launcher], args: argparse.Namespace
) -> dict[str, object]:
...
def render(report: dict[str, object], tools: dict[str, rivals.Launcher]) -> None:
    ...
    if fast:
        ...
        for key, row in fast["tools"].items():  # type: ignore[index]
            ratio = (
                f"{row['best'] / best:.1f}x"
                if best and row["verdict"] != "error"
                else "-"
            )
            print(
    ...
    if zig:
        ...
        print(
            f"  {'zig build-obj':<14} {theirs['verdict']:<12} {theirs['seconds']:>8.3f}s"
        )
        if theirs["detail"]:
